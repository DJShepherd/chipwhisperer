...
class CWGlitchParams(object):
    ...
    def get_repeat(self, gc, set):
        return set[gc.param_index(KEY_REPEAT)]

    def get_offset(self, gc, set):
        return set[gc.param_index(KEY_OFFSET)]

    def get_width(self, gc, set):
        return set[gc.param_index(KEY_WIDTH)]

    def get_extoff(self, gc, set):
        return set[gc.param_index(KEY_EXTOFF)]

    def get_glitcht(self, gc, set):
        return set[gc.param_index(KEY_GLITCHT)]
...
class CWLiteLoopVCCParams(CWGlitchParams):
    def __init__(self, vxx, params):
        if vxx:
            params[KEY_GLITCHT] = (1, 3, 1)
        super().__init__(params)

class CWLiteLoopVCCParams1(CWLiteLoopVCCParams):
...
class CWLiteLoopVCCParams2(CWLiteLoopVCCParams):
    def __init__(self, vxx):
        super().__init__(vxx, {
            KEY_OFFSET : (-45, -29, 0.4),
            KEY_WIDTH : (0, 38, 0.4),
            KEY_EXTOFF : (2, 10, 1),
            KEY_REPEAT : (1, 8, 1)
        })

class CWLiteLoopVCCParams3(CWLiteLoopVCCParams):
    def __init__(self, vxx):
        super().__init__(vxx, {
            KEY_OFFSET : (-41, -30, 0.4),
            KEY_WIDTH : (34.7, 36, 0.4),
            KEY_EXTOFF : (6, 6, 1),
            KEY_REPEAT : (7, 7, 1),
        })

# TODO: Move functionality to GlitchResults
class ResultRange(object):
    ...
    def display_range(self):
        gc_count = self._gc.get_count(self._group)
...
class SimpleSerialGlitchContext(cw.GlitchLoopGCContext, cw.GlitchLoopTargetContext):
    ...
    def samples_obtained(self):
        if self.success_sample <= 0:
            return False
        return self.gc.get_count(KEY_GLITCH) >= self.success_sample
    ...
    def reset(self, prt=True):
        ...
        self.set_rst_and_delay(cw.consts.PDI_MODE_LOW)
        self.set_rst_and_delay(cw.consts.PDI_MODE_HIGHZ)
        if self.target:
            self.target.flush()
...
class SimpleSerialGlitchLoop(cw.GlitchLoopRunner):
    def __init__(self, glitcher, args):
        ...
        if args.random:
            ...
            self._params = cw.SimpleSerialSeqParams()
...
if __name__ == '__main__':
    ...
    if args.glitch:
        if args.glitch == GLITCH_ARG_CLK:
            ...
            params = CWLiteLoopVCCParams2(args.glitch == GLITCH_ARG_VXX)
